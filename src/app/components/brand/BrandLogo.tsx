...
export function BrandLogo({ compact = false, light = true }: BrandLogoProps) {
  const titleColor = light ? brand.ivory : brand.ink;
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center", gap: compact ? 1.5 : 2, minWidth: 0 }}>
      <BrandMonogram size={compact ? 44 : 52} />
      <Box sx={{ minWidth: 0 }}>
        <Typography
          sx={{
            ...
            color: titleColor,
            fontSize: compact ? "0.9375rem" : { xs: "1rem", sm: "1.25rem", md: "1.5rem" },
            lineHeight: 1.1,
          }}
        >
          Vathy Frontier
